...
"""
    ...
    try:
        opts = getopt.getopt(argv,"ghs:n:t:",["help", "sub=", "ses=", "task=", "param=", "model=", 
    ...
    surf_files = lsutils.get_file_from_substring([sub, task, model], path_to_sub_surf, return_msg=None)
    surf_name = f'{sub}_{param}_{task}_{model}'
    if (surf_files==None) | overwrite:
        if verbose:
        ...
        r2_mask = prf_params[:,-1] > 0.1
        ecc_mask = ecc < 5
        total_mask = r2_mask * ecc_mask
        masked_pol = np.where(total_mask, pol, -10)
        lh_c = read_morph_data(opj(path_to_sub_surf,'lh.curv'))
        lh_masked_pol = masked_pol[:lh_c.shape[0]]
        rh_masked_pol = masked_pol[lh_c.shape[0]:]

        # now save results as a curve file
        write_morph_data(
            opj(path_to_sub_surf, f'lh.{surf_name}'),
            lh_masked_pol
        )

        write_morph_data(
            opj(path_to_sub_surf, f'rh.{surf_name}'),
            rh_masked_pol
        )        
    # Make custom overlay:
    # value - rgb triple...
    ...
    fview_cmd = f'''freeview -f lh.inflated:overlay=lh.{surf_name}:{overlay_custom_str} rh.inflated:overlay=rh.{surf_name}:{overlay_custom_str}'''
