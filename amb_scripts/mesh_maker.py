...
if not os.path.exists(mesh_dir):
    os.mkdir(mesh_dir)
def srf_to_ply(srf_file, rgb_vals, hemi=None, values=None):
    ...
    **kwargs:
    ...
    for ih in ['lh.', 'rh.']:
        ...
        if ih=='lh.':
            ...
            ply_str = srf_to_ply(srf_surf_file, display_rgb[:n_hemi_vx[0],:], hemi=ih, values=data) # lh
        else:
            # ply_str = obj_to_ply(obj_surf_file, display_rgb[n_hemi_vx[0]:,:]) # rh
            ply_str = srf_to_ply(srf_surf_file, display_rgb[n_hemi_vx[0]:,:],hemi=ih, values=data) # rh
