...
Args:
    -s (--sub=)         e.g., 01
    -m (--model=)       model (CRF, binary, straight)
    -t (--task=)        CSFLE,CSFRE
...
---------------------------------------------------------------------------------------------------
    ...
    ow = False
    try:
        opts = getopt.getopt(argv,"h:s:n:t:r:m:",[
    ...
    if (old_grid_params is None) or ow:
        ...
        if model=='binary':
            crf_exp_grid = np.array([0])
        else:
            crf_exp_grid = np.linspace(
                settings['csf_bounds'][f'crf_exp-{model}'][0],
                settings['csf_bounds'][f'crf_exp-{model}'][1],
    ...
    bounds = [
        ...
        (settings['csf_bounds'][f'crf_exp-{model}']),   # beta        
