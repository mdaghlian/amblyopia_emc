...
class AmbPlotter(Csf2EGetter):
    ...
    def csf_tc_plotV3(self, eye, idx, time_pt=None):
        ...
        if do_current_stim:
            ...
            fig, ax = plt.subplots(1, 3,gridspec_kw={'width_ratios': [2,1,6]})
            new_ax = [None] * 4
            new_ax[0] = ax[0]
            new_ax[1] = ax[1]
            new_ax[3] = ax[2]
        ...
        if do_scatter:
            ...
            if isinstance(dot_col, np.ndarray) & do_legend:
                ...
                if not isinstance(kwargs['dot_col'], np.ndarray): 
                    cb.set_label(kwargs['dot_col'])
        if do_line:
            self._plot_bin_line(X2plot, Y2plot, X2plot, axs=axs, line_col=self.plot_cols[p_eye],line_label=f'{p_eye}-{p_id}: corr={XY_corr:.3f}', **kwargs)                
        self._add_axs_basics(axs, **kwargs)    
        if return_vals:
